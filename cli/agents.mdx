...
<Info>
  If your harness speaks MCP natively, connect the [MCP server](/mcp/connect)
  directly instead — you get typed tool schemas and the client can surface
  confirmation prompts itself.
</Info>
...
For an autonomous phone conversation, resolve the agent first, name the destination to the user and wait for approval before starting it:

```bash
ringee voice-agents list --json
ringee voice-agents call <agentId> --to +13055550123 --var first_name=Carlos --yes --json
```
