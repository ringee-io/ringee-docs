...
| Code | Meaning | Fix |
|---|---|---|
| `MICROPHONE_DENIED` | Permission refused or unavailable | Serve over HTTPS, allow the mic, add `allow="microphone"` to iframes |
| `NO_AUDIO_DEVICE` | No usable input device | Ask the agent to connect a microphone |
| `AUDIO_PLAYBACK_BLOCKED` | Autoplay policy blocked audio | Start calls from a real click or tap |
| `TELNYX_CONNECTION_FAILED` | The provider connection dropped | Retryable |
| `NETWORK_ERROR` | Request failed in transit | Retryable |
| `TIMEOUT` | The operation exceeded its deadline | Retryable |

## Retry policy

`retryable` is `true` for known transient failures — rate limits, timeouts, network failures and provider connection failures.
